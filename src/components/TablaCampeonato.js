...
class TablaCampeonato extends React.Component {
  ...
  loadTabla = () => {
    this.props.partidos.forEach(p => {
      ...
      if (modT1) {
        this.campeonato.filter(team => team.id !== modT1._id)
        modT1 = {
          _id: modT1._id,
      ...
      this.campeonato.push(modT1)
      
      let modT2 = this.campeonato.find(team => team.id === t2._id)
      if (modT2) {
        this.campeonato.filter(team => team.id !== modT2._id)
        modT2 = {
          _id: modT2._id,
